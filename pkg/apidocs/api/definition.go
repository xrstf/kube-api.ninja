...
func NewDefinitions(config *Config, spec *loads.Document) (*Definitions, error) {
	s := &Definitions{
	...
	if err := LoadDefinitions(config, spec, s); err != nil {
		return nil, err
	}
...
func (d *Definition) initExample(config *Config) error {
	...
	path := filepath.Join(config.ReleaseDirectory, config.ExampleLocation, d.Name, d.Name+".yaml")
